...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
		          ...
		          case 'headache' | 'nerves':
		          {
						  ...
						  var bg:FlxSprite = new FlxSprite(-500, -170).loadGraphic(Paths.image('bgs/garStagebg'));
						  bg.antialiasing = true;
						  bg.scrollFactor.set(0.7, 0.7);
						  bg.active = false;
						  add(bg);

						  var bgAlley:FlxSprite = new FlxSprite(-500, -200).loadGraphic(Paths.image('bgs/garStage'));
		          ...
		          case 'release':
		          {
						  ...
						  var bg:FlxSprite = new FlxSprite(-500, -170).loadGraphic(Paths.image('bgs/garStagebgAlt'));
						  bg.antialiasing = true;
						  bg.scrollFactor.set(0.7, 0.7);
						  bg.active = false;
						  add(bg);

						  var bgAlley:FlxSprite = new FlxSprite(-500, -200).loadGraphic(Paths.image('bgs/garStageAlt'));
						  bgAlley.antialiasing = true;
						  bgAlley.scrollFactor.set(0.9, 0.9);
						  bgAlley.active = false;
						  add(bgAlley);

						  var corpse:FlxSprite = new FlxSprite(-230, 540).loadGraphic(Paths.image('bgs/gardead'));
		          ...
		          case 'fading':
		          {
						  ...
						  var bg:FlxSprite = new FlxSprite(-500, -170).loadGraphic(Paths.image('bgs/garStagebgRise'));
						  bg.antialiasing = true;
						  bg.scrollFactor.set(0.7, 0.7);
						  bg.active = false;
						  add(bg);

						  var bgAlley:FlxSprite = new FlxSprite(-500, -200).loadGraphic(Paths.image('bgs/garStageRise'));
						  bgAlley.antialiasing = true;
						  bgAlley.scrollFactor.set(0.9, 0.9);
						  bgAlley.active = false;
						  add(bgAlley);
	...
	function songOutro():Void
		{
			...
			if (isStoryMode)
			{
				...
				{
					case 'fading':
						LoadingState.loadAndSwitchState(new VideoState("assets/videos/daEnd.webm",new StoryMenuState()));
					default:
						endSong();
				}
			}
	...
	override function stepHit()
	{
		...
		if (dad.curCharacter == 'tom' && SONG.song.toLowerCase() == 'release')
		{
			if (curStep == 840)
			{
				dad.playAnim('lame', true);
			}
		}
